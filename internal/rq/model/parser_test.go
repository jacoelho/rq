...
`,
			check: func(t *testing.T, steps []Step) {
				...
				if got, _ := s.Query.Get("search"); got != "Install Linux" {
					t.Errorf("Query[search] = %q, want 'Install Linux'", got)
				}
				if got, _ := s.Query.Get("order"); got != "newest" {
					t.Errorf("Query[order] = %q, want 'newest'", got)
				}
				if got, _ := s.Query.Get("limit"); got != "10" {
					t.Errorf("Query[limit] = %q, want '10'", got)
				}
			},
		},
		{
...
`,
			check: func(t *testing.T, steps []Step) {
				assertSingleStep(t, steps, "GET", "https://api.example.com/health")
				if steps[0].When != "status_code == 200 && is_ready" {
					t.Fatalf("When = %q", steps[0].When)
				}
			},
		},
		{
...
`,
		...
		{
			name: "header_capture_unknown_key_not_allowed",
			yaml: `
- method: GET
  url: https://api.example.com/health
  captures:
...
`
	...
	if steps[0].BodyFile != "./payload.bin" {
		t.Fatalf("BodyFile = %q", steps[0].BodyFile)
	}
}
